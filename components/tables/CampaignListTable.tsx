...
export function CampaignListTable({ campaigns }: { campaigns: CampaignWithNPCs[] }) {
	...
	const sortedItems = useMemo(() => {
		return [...items].sort((a: CampaignWithNPCs, b: CampaignWithNPCs) => {
			...
			const second = b[sortDescriptor.column as keyof CampaignWithNPCs] as number;
	...
	const renderCell = useCallback((campaign: CampaignWithNPCs, columnKey: React.Key) => {
		switch (columnKey) {
			case 'campaign_name':
				return (
					<div className='flex flex-col'>
						<p className='capitalize hover:underline'>
							<Link href={`/campaigns/${campaign.id}`}>{campaign.campaign_name}</Link>
						</p>
					</div>
				);
			case 'description':
				return (
					<div className='flex flex-col'>
						<Tooltip delay={500} closeDelay={0} content={campaign.description} className='max-w-xs'>
							<p className='text-tiny capitalize truncate max-w-xs h-8'>{campaign.description}</p>
						</Tooltip>
					</div>
				);
			case 'npcs':
					...
					<div className='flex flex-col gap-1'>
						{campaign.npcs.length > 0 ? (
							campaign.npcs.map((npc) => (
								...
								>
									<Link key={npc.id} href={`/npcs/${npc.id}`} className='text-tiny'>
			...
			case 'actions':
				return (
					<div className='relative flex justify-center gap-2'>
						<DeleteModal idName='campaign_id' serverAction={deleteCampaignAction} id={campaign.id}>
	...
	return (
		...
		>
			<TableHeader className=''>
				<TableColumn allowsSorting align='center' key='campaign_name' maxWidth={48}>
					Name
				</TableColumn>
