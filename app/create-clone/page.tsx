    ...
    <div className='flex-1 w-full flex flex-col gap-20 items-center'>
      <div className='animate-in flex flex-col gap-20 opacity-0 max-w-4xl px-3'>
        ...
        <form action={formAction} className='flex flex-col gap-8'>
          ...
          />
          <input type='file' name='file' accept='audio/*' className='file-input' required />
          <SubmitButton text={'clone me'} className='btn-sm' />
        </form>
        {state?.message && <p className='p-2 text-center font-thin tracking-widest'>{state.message}</p>}
      </div>
    </div>
  );
}
